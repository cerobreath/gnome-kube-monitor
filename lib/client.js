...
// Common install locations across distros: the child PATH needs them for kubectl's
// exec auth plugins (krew puts oidc-login in ~/.krew/bin), findKubectl for kubectl.
/** @returns {string[]} */
 ...
 * @returns {string}  absolute path, or '' when nothing was found
 */
export function findKubectl(kubectlPath) {
    if (kubectlPath && isUsableProgram(kubectlPath))
        return kubectlPath;
    const onPath = GLib.find_program_in_path('kubectl');
    if (onPath)
        return onPath;
    // spawnv resolves a bare argv[0] against this process's PATH, not the one
    // the launcher sets, and gnome-shell's is trimmed, so a snap or krew install
    // is only reachable by probing the directories directly.
    for (const dir of extraPathDirs()) {
        const candidate = GLib.build_filenamev([dir, 'kubectl']);
        if (isUsableProgram(candidate))
            return candidate;
    }
    return '';
}
...
function buildArgv(extraArgv, {kubectlPath, context}, requestTimeout) {
    const argv = [findKubectl(kubectlPath) || 'kubectl'];
    if (requestTimeout)
...
async function runKubectl(extraArgv, opts, cancellable) {
    ...
    // Cancelling a poll must kill kubectl, whose SIGKILL does not reach an exec
    // auth plugin it spawned; buildEnviron is what keeps that one from hanging.
...
export class NodeWatcher {
    ...
    start() {
        ...
        let proc;
        try {
            proc = makeLauncher(this._opts.kubeconfig).spawnv(argv);
        } catch (e) {
    ...
    async _readLines(base, onLine) {
        ...
        try {
            await this._pump(stream, onLine);
        } finally {
            // close_base_stream releases the fd at finalization, so without this
            // every respawn holds two pipes open until the next GC.
            stream.close(null);
        }
    }
