...
//
// The banner wording lives here too, beside the lifecycle that decides when to
// say it, and goes through the gi-free wrappers in i18n.js. Node names inside
// those sentences are never translated -- they are cluster identifiers.
...
export function reduce(prevState, observation, config, nowMs) {
    ...
    // --- ClusterUnreachable (always evaluated) ---
    {
        ...
        const {rec, action} = stepAlert(prev, !observation.reachable, nowMs, {
            ...
            key: CLUSTER_KEY,
            // Translators: stands in for a node name when a grouped banner
            // lists what changed, e.g. "web-1, db-2, cluster". Lowercase, to
            // read as one item in that list.
            label: _('cluster'),
            fireTitle: observation.error?.title ?? errorTitle('unreachable'),
            // Deliberately no body: kubectl's raw detail can carry credential
            // material from an exec plugin's stderr, and GNOME renders
            // notification bodies on the lock screen. The detail stays in the
            // menu, which the user has to open. (model.js redacts it too --
...
/**
 * Bound the tracked-alert map. It only ever holds currently-degraded alerts, but
 * on a big cluster that bound *is* the cluster size: a rolling reboot of 5000
 * nodes would otherwise persist a multi-megabyte blob to GSettings. Firing
 * records are kept ahead of pending ones so the cap sheds the least interesting
 * entries first.
...
function capAlerts(alerts) {
    ...
    if (keys.length <= MAX_TRACKED_ALERTS)
        return alerts;
    // Reserve the cluster alert outside the ranking rather than special-casing it
    // inside the comparator: which arm of such a check runs depends on Array.sort
    // internals, so keeping it out makes the behaviour both clearer and testable.
...
function stepNode(prev, cond, name, nowMs, config, settle) {
    return stepAlert(prev, cond, nowMs, {
        ...
        // %s is the node's name, as the cluster spells it -- do not translate it.
...
export function groupActions(actions) {
    ...
    if (fires.length)
        out.push(coalesce(fires, 'fire', 'critical'));
    if (resolves.length)
...
function coalesce(list, type, urgency) {
    ...
    return {title, body: list.map(a => a.label).join(', '), urgency};
}
...
export function deserializeState(json) {
    ...
    try {
        ...
        // ACL, so any same-UID process can rewrite it: an unknown `phase` would
        // fall through to the firing branch (spurious banner whose title is the
        // attacker's key), and a NaN timestamp would make every elapsed()
        // comparison false so the alert could never resolve. Drop what doesn't
        // typecheck instead of trusting it.
        /** @type {Record<string, AlertRecord>} */
        const alerts = {};
