...
export const KubeIndicator = GObject.registerClass({
    ...
    _buildMenu() {
        ...
        this._titleLabel = new St.Label({text: 'Loading…', style_class: 'kube-title'});
        ...
        this._contextSubmenu = new PopupMenu.PopupSubMenuMenuItem('Cluster', false);
        menu.addMenuItem(this._contextSubmenu);
    ...
    setContexts(contexts, current) {
        this._contextSubmenu.menu.removeAll();
        if (contexts.length === 0) {
            this._contextSubmenu.menu.addMenuItem(
        ...
        for (const ctx of contexts) {
            ...
            item.connect('activate', () => this.emit('context-selected', ctx));
            this._contextSubmenu.menu.addMenuItem(item);
        }
    }
    ...
    update(state) {
        ...
        if (state.context) {
            ...
            this._contextSubmenu.label.text = `Cluster: ${state.context}`;
        }
    ...
    _renderNodes(nodes) {
        ...
        if (sorted.length === 0) {
            this._nodesSection.addMenuItem(
                new PopupMenu.PopupMenuItem('No nodes found', {reactive: false}));
            return;
        }
        for (const node of sorted) {
    ...
    _makeNodeItem(node) {
        ...
        const subLabel = new St.Label({style_class: 'kube-node-sub'});
        info.add_child(subLabel);
        item.add_child(info);

        const row = {item, dot, nameLabel, valueLabel, subLabel};
        this._updateRow(row, node);
    ...
    // Refresh only the parts of a row that change between polls of the same
    // node set: the up/down duration and the CPU/MEM sub-line. Level-dependent
    // styling (dot colour, value colour) can't change here — a level change
    // bumps the signature and forces a rebuild instead.
    /**
     * @param {NodeRow} row
     * @param {import('./model.js').DetailNode} node
     */
    _updateRow(row, node) {
        ...
        if (node.cpuPct != null || node.memPct != null) {
            const cpu = node.cpuPct != null ? `${node.cpuPct}%` : '—';
            const mem = node.memPct != null ? `${node.memPct}%` : '—';
            row.subLabel.text = `CPU ${cpu}   ·   MEM ${mem}`;
    ...
    _renderPods(pods) {
        ...
        if (pods.crashloop > 0)
            add(`  ·  ${pods.crashloop} crash`, 'kube-pods-err');
        if (pods.failed > 0)
