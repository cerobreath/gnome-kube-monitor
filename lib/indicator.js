...
// compositor's main loop, so this is what stops a large cluster from freezing
// the desktop when the menu opens. Rows are sorted most-severe-first and the
// remainder is summarised, never silently dropped.
//
// Measured, not guessed (Ryzen 7 5800HS, headless GNOME 50; see CLAUDE.md):
// 50 rows cost ~40 ms to build and ~130 ms to reach a laid-out frame, and that
// stays flat however big the cluster is. Without the cap the same menu costs
// ~640 ms at 200 nodes and ~1.8 s at 1000 -- a visible freeze of the whole
// shell, every time the menu opens. Re-rendering an unchanged list is ~2.5 ms,
// so the cost is paid on the first open and on a signature change, not per poll.
// Roughly 2.5 ms per row: halving this halves the hitch, at the price of showing
// fewer nodes to someone who opened the menu to look at them.
...
export const KubeIndicator = GObject.registerClass({
    ...
    _buildMenu() {
        ...
        //
        // `reactive: true, activate: false` rather than the obvious
        // `reactive: false`: St ties the `:insensitive` pseudo-class to an
        // actor's reactivity, and the shell's theme paints
        // `.popup-inactive-menu-item:insensitive` in its disabled grey (#9b9b9d
        // on dark, #78787b on light) — which this container then passes down to
        // the live buttons inside it, so a working cluster switcher rendered as
        // if it were greyed out. Keeping the item reactive but non-activatable
        // leaves it matching plain `.popup-inactive-menu-item`, i.e. the theme's
        // ordinary foreground, and costs nothing: PopupBaseMenuItem only wires
        // its click gesture and its hover-to-active binding when `activate` and
        // `hover` are set. Doing it this way rather than with a colour of our
        // own is what keeps the Light style correct — hardcoding #ffffff here is
        // the exact regression that once made these rows white-on-white.
        ...
        // PopupBaseMenuItem passes `track_hover: params.reactive` to St, so the
        // line above would otherwise have St set `:hover` on this container and
        // the theme paint `.popup-menu-item:hover` across the whole list block
        // (measured: #535359) behind the row the pointer is actually on. The
        // rows do their own highlighting; the container must stay invisible.
