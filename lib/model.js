...
//
// The few strings it does produce are user-facing, so they go through the
// gi-free wrappers in i18n.js (identity until a process binds a locale). What
// stays untranslated is deliberate: Kubernetes' own vocabulary -- "Ready",
// "NotReady", "SchedulingDisabled", the pressure condition types, role names --
// is API identifiers, printed verbatim by `kubectl get nodes`. Translating them
// would make the menu disagree with the command it is a window onto.
 ...
 * has. Shared by every duration the extension shows (node up/down time, "updated
 * N ago", the remaining mute) so translators write each unit once.
 *
 * The `duration` context is what tells them these are abbreviations rather than
 * prose: on their own, "m" is as likely to be metres as minutes.
 * @param {number} seconds
 * @returns {string}
 */
export function formatDuration(seconds) {
    if (seconds < 60) {
        // Translators: abbreviated duration in seconds, e.g. "45s". Keep it as
        // short as your language allows -- it shares a fixed-width menu row with
        // the node name. %d is the number of seconds.
 ...
 *   callers that need to branch on the cause rather than re-match the prose.
 * @property {string} title   Short human headline (our wording), translated.
 * @property {string} detail  kubectl's own words, de-noised; '' when none useful.
 *   Never translated: it is kubectl's output, not ours to rewrite.
 */

/** @typedef {keyof typeof ERROR_TITLE} ErrorKey */

// Human headline per category. Kept beside the classifier (not in the view) so
// the wording travels with the matching logic and its tests. Marked with N_()
// rather than _(): this table is built at module load, long before a process has
// bound a locale, so it stores the English source and classifyError() translates
// at lookup time.
const ERROR_TITLE = {
    ...
    tls: N_("The cluster's certificate didn't check out"),
    // Translators: the API server rejected the credentials (HTTP 401).
    unauthorized: N_('The cluster turned down the login'),
    // Translators: an expired token that a credential helper must refresh.
    login: N_('The login needs renewing'),
    ...
    badContext: N_("That context isn't set up"),
 ...
 * applies it to every diagnostic line as a backstop: the journal is readable and
 * long-lived, so a token must not be able to reach it even by mistake.
 * @param {string} s
 * @returns {string}
 */
...
export function classifyError(raw, {timedOut = false} = {}) {
    ...
    if (timedOut)
        return {key: 'timeout', title: errorTitle('timeout'), detail: ''};
