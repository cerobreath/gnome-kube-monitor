...
/**
 * Per node: how long it has been alive/dead, prefixed with the most relevant
 * qualifier — role when healthy, the failure reason when degraded.
 * @param {{ready: boolean, level: NodeLevelValue, roles: string[], since: string, issues: string[]}} node
 * @returns {string}
 */
export function nodeValue(node) {
    const dur = node.ready ? `up ${node.since}` : `down ${node.since}`;
    if (node.level === NodeLevel.OK)
        return `${node.roles.join(', ')} · ${dur}`;
    if (!node.ready)
        return dur;
    return `${node.issues.join(', ')} · ${dur}`;
}
