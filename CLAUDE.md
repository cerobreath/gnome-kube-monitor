...
build step or bundler**; gnome-shell loads the `.js` files directly. There is now a
dev-only test/lint layer (`package.json`, `tests/`, `eslint.config.js`) run with node; it
is not shipped with the extension.
...
- **`lib/poller.js` — the poll loop.** Owns tier selection, backoff, the watchdog, and
  reentrancy (see invariants below). Pushes render state out via callbacks.
...
- `model.js` is the only thing with real unit tests (`tests/model.test.js` + hand-built
  `tests/fixtures.js` covering every node branch). Run with `npm test`.
- Types are JSDoc + the `@girs/*` packages (dev-only; the shipped extension is still plain
  ...
  the logic (`model`/`client`/`poller`/`prefs`); `tsconfig.ui.json` relaxes only
