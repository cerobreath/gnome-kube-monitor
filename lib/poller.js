...
import {applyMetrics, firstLine} from './model.js';
 ...
 * @property {(nodes: {name: string, ready: boolean}[]) => void} [onNodes]
 */
...
export class KubePoller {
    ...
    // Manual refresh (refresh button, or a connection setting changed).
    refreshNow() {
    ...
    _scheduleNext(delaySec) {
        this._timerId = GLib.timeout_add_seconds(
            GLib.PRIORITY_DEFAULT, Math.max(0, Math.round(delaySec)), () => {
                this._timerId = 0;
                this._tick();
    ...
    async _tick() {
        ...
        try {
            ...
            this._failures = 0;
            this._deps.onNodes?.(state.nodes);
            this._deliver(state);
        } catch (e) {
            ...
            this._deliver({
                ...
                level: 'error',
                error: timedOut ? 'Cluster unreachable (timed out)' : firstLine(err?.message ?? err),
                nodes: [],
                readyCount: 0,
                total: 0,
                pods: null,
            ...
            this._polling = false;
            if (!this._stopped) {
                // If the menu opened mid-poll, refreshNow() was blocked by the
                // reentrancy guard and we just finished a cheap health poll —
                // fetch detail now instead of waiting a whole interval.
