...
function buildEnviron(kubeconfig) {
    // Start from an allowlist rather than gnome-shell's whole environment. The
    // shell inherits the `systemd --user` env, which routinely holds
    // SSH_AUTH_SOCK, GNOME_KEYRING_CONTROL and whatever `environment.d` /
    // `systemctl --user set-environment` put there (people do stash tokens
    // that way). kubectl passes its env straight to exec credential plugins,
    // so forwarding all of it hands third-party binaries the session's secrets.
    // Everything kubectl legitimately needs is listed here.
    const KEEP = [
    ...
    // Cover the common install locations across distros so auto-detect finds
    // both kubectl and its exec auth plugins (e.g. krew-installed oidc-login):
    // system dirs, Snap, ~/.local/bin, and krew's ~/.krew/bin.
...
/**
 * Pick the kubectl binary. An explicit setting wins, but only if it actually
 * looks like a program: the value comes from GSettings, and dconf has no
 * per-key ACL, so any same-UID process (including a sandboxed app with
 * gsettings access) can write it. Without this check that would be a
 * "get gnome-shell to exec my binary every N seconds" gadget. Requiring an
 * absolute path to an existing, executable regular file keeps the user's
 * legitimate override working while refusing the obviously-wrong shapes.
...
async function runKubectl(extraArgv, {kubectlPath, kubeconfig, context}, cancellable) {
    ...
    // from settings, and this form can't be re-read as a flag no matter how the
    // arg parser treats a leading dash.
    if (context)
    ...
    try {
        ...
        if (!proc.get_successful()) {
            // Whatever kubectl said, verbatim -- and nothing at all when it said
            // nothing. classifyError renders this as the error *detail*, whose
            // whole contract is "kubectl's own words", so it is never translated;
            // inventing an English sentence to fill the slot would both break
            // that contract and put untranslatable text in front of the user.
            // An empty message classifies to the generic headline with no detail,
            // which is the same thing the watchdog path does.
