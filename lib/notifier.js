...
// and moved both Source and Notification to params-object constructors.
// Feature-detect it (addNotification exists on 46+, not on 45) so the same code
// path is chosen on every supported shell, present and future.
//
// Probed per call rather than cached at import: the check is a single typeof, the
// answer cannot change within a running shell anyway, and keeping it out of
// module scope means no import-time state -- which also lets both branches be
// covered by tests in one process.
/** @returns {boolean} */
